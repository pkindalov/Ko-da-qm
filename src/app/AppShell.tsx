...
import { useDisableAccount } from '../features/profile/hooks/useDisableAccount';
...
export const AppShell = () => {
  ...
  const { disableAccount, isDisabling } = useDisableAccount(tweaks.lang);
  ...
  const screens: Record<Tab, React.ReactNode> = {
    ...
    cookbook: <ErrorBoundary><CookbookScreen recipes={recipes} favoriteIds={favoriteIds} profile={profile} lang={tweaks.lang} onEditRecipe={(recipe) => { setPendingOpenRecipeId(recipe.id); setTab('recipes'); }} onDeleteRecipe={removeRecipe} /></ErrorBoundary>,
    ...
    planner: <ErrorBoundary><PlannerScreen recipes={recipes} fridge={fridge} products={products} profile={profile} lang={tweaks.lang} planner={planner} setPlanner={setPlanner} favoriteRecipes={favoriteRecipes} onSaveSuggestion={(recipe) => addRecipe({ ...recipe, authorName: profile.name, authorEmail: userEmail })} onViewRecipe={(id) => { setTab('recipes'); setPendingOpenRecipeId(id); }} onDeleteRecipe={removeRecipe} /></ErrorBoundary>,
  };
